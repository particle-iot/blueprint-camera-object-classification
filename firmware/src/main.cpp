...
#include <camera-object-detection_inferencing.h>
...
// Camera is connected to Serial1
// Reset pin is D2
// Timeout is 500 ms
uCamIII<USARTSerial> ucam(Serial1, D2, 500);

// Camera baud rate
const int CAMERA_BAUD = 115200;
 ...
 */
int raw_feature_get_data(size_t offset, size_t length, float *out_ptr) {
    // Read picture data from camera here    
    return 0;
}
...
void setup()
{
    ...
    long retVal = ucam.init(CAMERA_BAUD);
    if (retVal <= 0) {
        _log.error("camera init failed");
    }

    retVal = ucam.setImageFormat(uCamIII_RAW_8BIT, uCamIII_128x128);
    if (retVal == 0) {
